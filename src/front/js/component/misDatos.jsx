...
export const MisDatos = () => {
  ...
  const { state, actions } = useOutletContext();
  ...
  const handleSubmit = async (e) => {
     ...
     const response = await actions.changePassword(passwordInput);
      if(response.success){
        setSuccessOperation('Contraseña cambiada con exito')
        setPasswordInput('')
        setSecondPasswordInput('')
    ...
    <>
      <div className="p-2">
        <div className="relative flex flex-row gap-5">
          ...
          <span className="font-regular text-base">{state.current_user.email}</span>
        </div>
        ...
        <div className="flex justify-center w-secreen relative mt-5">
          <button onClick={() => {setCambiarContraseña(!cambiarContraseña)}} className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 bg-button-background text-white py-2 px-4 rounded">
            Cambiar Contraseña
          </button>
        </div>
        <form onSubmit={(e) => handleSubmit(e)}>
        <div className={`mb-6 mt-10 ${cambiarContraseña === true ? '' : 'hidden'}`}>
          ...
          <div className="relative -mt-10">
            <svg
              ...
              stroke-width="1.5"
              stroke="currentColor"
              class="size-8 relative pointer-events-none w-8 h-8 top-[37px] left-[12px]"
            >
              <path
        ...
        <div className={`mb-6 mt-3`}>
          ...
          <button type='submit' className=" mt-5 w-full md:w-1/2 lg:w-1/3 xl:w-1/4 bg-button-background text-white py-2 px-4 rounded">
