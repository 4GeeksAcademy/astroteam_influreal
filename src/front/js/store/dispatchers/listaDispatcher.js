...
export const addInfluencerToListaDispatcher = async (
  token,
  listaId,
  influencerId
) => {
  const response = await fetch(
    `${process.env.BACKEND_URL}listas/${listaId}/add_influencer`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
...
export const removeInfluencerFromListaDispatcher = async (
  token,
  listaId,
  influencerId
) => {
  const response = await fetch(
    `${process.env.BACKEND_URL}listas/${listaId}/remove_influencer`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
...
export const selectSingleListDispatcher = async (token, listaId) => {
  try {
    const response = await fetch(
      `${process.env.BACKEND_URL}listas/${listaId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
