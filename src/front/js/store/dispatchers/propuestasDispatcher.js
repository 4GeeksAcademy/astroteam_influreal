...
export const updatePropuestaDispatcher = async (
  token,
  propuestaId,
  nombre,
  descripcion
  ...
  const response = await fetch(
    `${process.env.BACKEND_URL}propuestas/${propuestaId}`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
...
export const loadPropuestaDispatcher = async (propuestaId, token) => {
  ...
  try {
    ...
    return {
      success: false,
      message:
        "Error de red al obtener la propuesta. Por favor, intenta de nuevo.",
    };
  }
};
...
export const sendPropuestaDispatcher = async (token, lista, propuesta) => {
  try {
    const response = await fetch(
      `${process.env.BACKEND_URL}propuestas/enviar`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
